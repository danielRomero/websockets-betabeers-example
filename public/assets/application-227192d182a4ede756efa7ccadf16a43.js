...
(function() {
  ...
  window.websocketsExample.init = function(domain) {
    return window.websocketsExample.domain = domain;
  };

  window.websocketsExample.start = function() {
    if ((window.websocketsExample.user_name === void 0) || window.websocketsExample.user_name === "") {
      return alert("Elige un nombre de usuario primero");
    } else {
      window.websocketsExample.dispatcher = new WebSocketRails("" + window.websocketsExample.domain + "/websocket");
      return window.websocketsExample.bind_events();
    }
  };

  window.websocketsExample.bind_events = function() {
    window.websocketsExample.dispatcher.on_open = function(data) {
    ...
    return window.websocketsExample.dispatcher.bind('users_count_changed', window.websocketsExample.users_count_changed);
  };

  window.websocketsExample.new_message_received = function(response) {
    $('#chat_messages_index').append("<li class='left clearfix message' data-time=" + response.datetime + "> <div class='chat-body clearfix'> <div class='header'> <strong class='primary-font'>" + response.user_name + "</strong> <small class='pull-right text-muted'> <span class='glyphicon glyphicon-time'></span> <span class='datetime-ago'>" + (window.websocketsExample.timeSince(response.datetime)) + "</span> </small> </div> <p> " + response.message + " </p> </div> <hr> </li>");
    if ($('li.left.clearfix.message').length > 0) {
      return $('li.left.clearfix.message')[$('li.left.clearfix.message').length - 1].scrollIntoView();
    }
  };

  window.websocketsExample.connection_success = function(response) {
    $('#chat_messages_index').append("<li class='left clearfix'> <div class='chat-body clearfix text-center text-muted'> <small> <p> " + response.message + " </p> <small> </div> <hr> </li>");
    ...
    return window.websocketsExample.calc_timeSince();
  };

  window.websocketsExample.users_count_changed = function(response) {
    return $('#connected_users_count').html("" + response.users_count + " usuarios conectados");
  };

  window.websocketsExample.send_message = function(message) {
    var object_to_send;
    if ((message !== void 0) && (message !== '')) {
      object_to_send = {
        user_name: window.websocketsExample.user_name,
        message: message,
        datetime: new Date()
      };
  ...
  window.websocketsExample.enable_to_send = function() {
    if ($('#name_input').val().length > 2) {
      window.websocketsExample.user_name = $('#name_input').val();
      if (!window.websocketsExample.connected) {
        return $('#btn_connect_chat').removeClass('disabled');
      } else {
        $('#btn_connect_chat').addClass('disabled');
        $('#btn_connect_chat').html('¡Conectado!');
        $('#btn_send_chat').removeClass('disabled').html('Enviar');
        return $('#message_input').removeAttr('disabled');
      }
    } else {
      if (!window.websocketsExample.connected) {
        return $('#btn_connect_chat').addClass('disabled');
      } else {
        $('#btn_connect_chat').addClass('disabled');
        $('#btn_connect_chat').html('¡Conectado!');
        $('#btn_send_chat').addClass('disabled').html('Nombre corto');
        return $('#message_input').attr('disabled', 'disabled');
      }
    }
  };

  window.websocketsExample.timeSince = function(date) {
  ...
  window.websocketsExample.calc_timeSince = function() {
    return setInterval((function() {
      $("li.left.clearfix.message").each(function(index) {
